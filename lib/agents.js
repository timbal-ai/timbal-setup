...
const MCP_URL = 'https://api.dev.timbal.ai/mcp';
  ...
  return {
    ...
    writeMcp(token) {
      try {
        ...
        const entry = { [urlField]: MCP_URL };
        if (token) {
          entry.headers = { Authorization: `Bearer ${token}` };
        }
        config[serversKey][MCP_KEY] = entry;
...
const claudeCode = {
  ...
  writeMcp(token) {
    ...
    const { ok, stderr } = run('claude', args);
...
const codex = {
  ...
  writeMcp(token) {
    ...
    const args = ['mcp', 'add', MCP_KEY, '--url', MCP_URL];
    if (token) {
      args.push('--bearer-token-env-var', 'TIMBAL_API_KEY');
    }

    const { ok } = run('codex', args);
...
const geminiCli = {
  ...
  writeMcp(token) {
    ...
    args.push(MCP_KEY, MCP_URL);

    const { ok } = run('gemini', args);
