# Timbal App Integration — API Reference
...
> **Working on the UI?** See `references/ui.md` for the full frontend reference (React, Vite, Shadcn, chat interface, styling, runtime environment constraints).
...
For the full frontend reference (project structure, chat interface, styling, streaming, runtime constraints), see **`references/ui.md`**.

The key rule: the UI calls `/api/*` for everything. It has no Timbal SDK dependency and no direct platform access. Use `authFetch` from `@/auth/tokens` for all requests. Never modify `src/auth/`.
